...
// 1. Check if we are running in a development environment
const isDev = self.location.hostname === 'localhost' || self.location.hostname.includes('ais-dev-') || self.location.hostname === '127.0.0.1';

if (isDev) {
  // During development, clear all caches immediately and bypass caching to prevent stale assets
  console.log('[SW] Development mode detected: disabling service worker caching.');
    ...
    event.waitUntil(
      caches.keys().then((keys) => {
        return Promise.all(keys.map(key => {
          console.log('[SW] Dev mode: deleting cache', key);
  ...
  self.addEventListener('fetch', (event) => {
    // No-op: bypass caching entirely and let browser perform standard network queries
    return;
  });
} else {
  // Production Service Worker Caching logic
...
// Suporte a Notificações Push (suportado em ambos se configurado)
